...
function resultado() {
    ...
    if (darf > 0 && darf < 10) {
        ...
        res.innerHTML = `<p>Seu prejuízo na operação foi de R$${prejuizo.toFixed(2)} e você poderá contabilizar esse prejuízo com outras vendas futuras para compensação.</p>`
    }
}
...
function resultado2() {
    ...
    res2.innerHTML = `<p>Seu yield on cost foi de ${yocost.toFixed(3)}% nesse mês.</p>`
    res3.innerHTML = `<p>A rentabilidade anualizada seria ${(yocost * 12).toFixed(3)}%.</p>`
}